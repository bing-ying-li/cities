...
function averagePopulation(data, island) {
  const filteredCities = _.filter(data, { island });
  const totalPopulation = _.reduce(filteredCities, (sum, city) => sum + city.population, 0);
  return totalPopulation / filteredCities.length;
}

function biggestAndSmallest(data, island) {
  const filteredCities = _.filter(data, { island });
  const biggest = _.max(filteredCities, city => city.population);
  const smallest = _.min(filteredCities, city => city.population);
